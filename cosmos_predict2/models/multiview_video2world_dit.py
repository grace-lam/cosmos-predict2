...
class MultiViewCrossAttention(Attention):
    ...
    def forward(self, x, context=None, rope_emb=None):
        ...
        B, L, D = x.shape
        # TODO: Make this more general beyond 512 text tokens
...
class MultiViewBlock(Block):
    ...
    def __init__(
        ...
        super().__init__(
            x_dim, context_dim, num_heads, mlp_ratio, use_adaln_lora, adaln_lora_dim, backend, image_context_dim
        )
        self.state_t = state_t
        if image_context_dim is None:
            del self.cross_attn
            self.cross_attn = MultiViewCrossAttention(
                x_dim, context_dim, num_heads, x_dim // num_heads, qkv_format="bshd", state_t=state_t
            )
        else:
...
class MultiViewDiT(MinimalV1LVGDiT):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            *args, mlp_ratio=mlp_ratio, crossattn_emb_channels=crossattn_emb_channels, sac_config=sac_config, **kwargs
        )
        del self.blocks
